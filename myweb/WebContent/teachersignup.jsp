...
//	alert(ucN);
	if(!ucNnumber.test(ucN)){
		...
		if(ucN.length <= 0 && ucN.length >= 11){
			...
			//通过ajax校验该工号是否已经被注册
			var url = "${pageContext.request.contextPath}/teacher_checkNumber.action";
			var param = {"user_number":ucN};
			$.post(url,param,function(data){
				// 操作data，进行判断
				if(data && data == "no"){
					// 提示
					$("#checkUID").addClass("error");
					$("#checkUID").html("工号已经存在");
				}else{
...
<FORM id=form1 name=form1 action="${ pageContext.request.contextPath }/teacher_teacherSignUp.action" method=POST target=_self onsubmit="return validateSignUp()">
<TABLE id = "surroundpartern" >
	...
	<TD class = "signTable" style = "WIDTH: 450px; HEIGHT: 300px">
		<TABLE cellSpacing=0 cellPadding=5 width=450px align=left border=0>	
		  <TR>
		    <TD id = cName >姓名：</TD>
		    <TD><INPUT id = txtName style = "WIDTH: 130px" type = text name = "teacher_name" onchange = "comfirmUserName(this.value)" maxLength="8"><span id = confName ></span></TD>
		  </TR>
		  <TR>
		    <TD id = cName >工号：</TD>
		    <TD><INPUT id = txtId style = "WIDTH: 130px" type = text name = "teacher_number" maxLength="10" onchange = "comfirmUserID(this.value)"><span id = checkUID ></span></TD>
		  </TR>
		  <TR>
		    <TD id = cName >密码：</TD>
		    <TD><INPUT id = txtPassword style = "WIDTH: 130px" type = password name = "teacher_password" onchange = "checkFunction(this.value)" maxLength="16"><span id = checkPswd >&nbsp;密码长度：4-16位</span></TD>
		  </TR>
		  <TR>
		    <TD id = cName >密码确认：</TD>
		    <TD><INPUT id = txtConfirmPassword style = "WIDTH: 130px" type = password name = "user_recheckpassword" onchange = "comfirmFunction()" maxLength="16"><span id = confPswd ></span></TD>
		  <TR>
