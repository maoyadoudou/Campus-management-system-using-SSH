...
<STYLE type="text/javascript">
	...
	$(document).ready(function(){$("FORM").submit(function(){
		if ($('input:radio:checked').val()=="teacher"){
		   $("FORM").attr("action","${ pageContext.request.contextPath }/teacher_changePassword.action");
		   $("FORM").submit();
		}
		else
		{
		   $("FORM").attr("action","${ pageContext.request.contextPath }/student_changePassword.action");
		   $("FORM").submit();
		}
		})
	});
...
<FORM id=form1 name=form1 action="${ pageContext.request.contextPath }/user_changePassword.action" method=post target="main" onsubmit="return validateSignUp()">
