...
//如果原先已经修改过个人信息，则在修改页面的input中将原个人信息设置为默认信息
	$(document).ready(function(){
		...
		$.post(url,param,function(data){
			...
			if(data[0].student_avatar == null){
				$("#student_avatar").attr("src","${pageContext.request.contextPath }/images/default.jpg");
			}else{
				$("#student_avatar").attr("src",data[0].student_avatar);	
			}
			//性别
			var sex = data[0].student_sex
			var sexradio = document.getElementsByName("student_sex");
			if(sex != null ){
				if(sex == "男"){
			...
			//个人电话
			var phone = data[0].student_phone;
			if(phone != null ){
				document.form1.student_phone.value=phone;
		    }
		    //个人邮箱
		    var mail = data[0].student_mail;
			if(mail!=null){
				document.form1.student_mail.value=mail;
			}
			//出生日期
			var date_year = 0;
			var date_month = 0;
			var date_day = 0;
			//加载填写了生日日期
			var birthday = data[0].student_birthday;
			//如果不为空，则提取年月日
			if(birthday != null){
	...
	$(document).ready(function(){
		$("#form1").submit(function(){
			...
			document.getElementById("student_birthday").setAttribute("value",year + month + day);
		    return true;
		});
	});
</script>
...
<FORM id=form1 name=form1 action="${ pageContext.request.contextPath }/student_changePersonalProfile.action" method=post target="main" enctype="multipart/form-data">
	<INPUT id = txtName type = hidden value=${userLogin.student_id} Name = "student_id" >
	<INPUT id = txtName type = hidden value=${userLogin.student_name} Name = "student_name" >
	<INPUT id = txtName type = hidden value=${userLogin.student_state} Name = "student_state" >
	<INPUT id = txtName type = hidden value=${userLogin.student_number} Name = "student_number" >
	<INPUT id = txtName type = hidden value=${userLogin.student_position} Name = "student_position" >
	...
	<TD class = "signTable" style = "WIDTH: 450px; HEIGHT: 300px">
		<TABLE cellSpacing=0 cellPadding=0  border=2 style = "WIDTH: 450px; HEIGHT: 600px">
			...
			<TR>
				...
				<TD align=center>
				<INPUT style = "WIDTH: 30px" type = radio name = "student_sex" id="manradio" value = "男" >男
			    <INPUT style = "WIDTH: 30px" type = radio name = "student_sex" id="womanradio" value = "女" >女
			    </TD>				
			</TR>
			<TR>
			...
			<TR>
			    ...
			    <input type="hidden" id="student_birthday" name="student_birthday" >
			    </TD>				
			</TR>			
			<TR>
				<TD align=center>电话：</TD>
				<TD colspan=2 align=center><INPUT id ="student_phone" style = "WIDTH: 300px" type = text name = "student_phone" maxLength="20"></TD>				
			</TR>	
			<TR>
				<TD align=center>Email：</TD>
				<TD colspan=2 align=center><INPUT id = student_mail style = "WIDTH: 300px" type = text name = "student_mail" maxLength="100"></TD>				
			</TR>			
			<TR>
				<td colspan=3>
...
//将选好的出生年月日整合后写入id为student_birthday的标签中
 	function writeBirthday(){
 		var birthday = "${userLogin.student_birthday}";
 		if(document.getElementById("sYear")[document.getElementById("sYear").selectedIndex].value == null ||
 		 document.getElementById("sMonth")[document.getElementById("sMonth").selectedIndex].value == null ||
 		  document.getElementById("sDay")[document.getElementById("sDay").selectedIndex].value == null){
 			document.getElementById("student_birthday").setAttribute("value",birthday);
 		}else{
 		var year = document.getElementById("sYear")[document.getElementById("sYear").selectedIndex].value + "-";
 		var month = document.getElementById("sMonth")[document.getElementById("sMonth").selectedIndex].value + "-";
 		var day = document.getElementById("sDay")[document.getElementById("sDay").selectedIndex].value;
